...
def do_elasticsearch(set_entities_tag_and_id):
    ...
    for word, label in set_entities_tag:
        ...
        if response:
            ...
            final_set.add((word,label,response_result))
...
def check_first_k_else_all(set_entities_tag_and_elastic_and_id):
    ...
    for entity_word, label, response in set_entities_tag_and_elastic:
        ...
        if not cancel_search:
            final_set.add( (word,label,list(id_labels),elastic_scores, matching_ratio) )
...
def queries_and_get_best_id(set_info_and_id):
    ...
    query_label_order = {"GPE": ["GPE", "ORGANIZATION", "PERSON", "OTHER"],
                         "GSP": ["GPE", "ORGANIZATION", "PERSON", "OTHER"],
                         "LOCATION": ["GPE", "ORGANIZATION","PERSON", "OTHER"],
                         "PERSON": ["PERSON", "GPE", "ORGANIZATION", "OTHER"],
                         "ORGANIZATION": ["ORGANIZATION", "GPE", "PERSON", "OTHER"],
                         "FACILITY": ["ORGANIZATION","GPE","PERSON", "OTHER"],
                         }
    r = 10
    small_value = 0.5
    title_multiplier = 3
    ...
    for word_info in set_info:
        word, label, elastic_id_labels, elastic_scores, matching_ratio = word_info
